CREATE OR REPLACE PROCEDURE       Snp_P_Crnewnomsyk_Vz
(
...
--	char_:=to_char(i);
	FOR sp_group IN
    (SELECT NRN,SGROUP_CODE FROM V_DICGNOMN
	WHERE ncrn = 187790521)

	LOOP
	IF mas.sgroup_code = sp_group.sgroup_code THEN
	flag_group := TRUE;
	END IF;
	END LOOP;
