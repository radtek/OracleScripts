...
P_EXCEPTION( 0,'»зменение значени€ пол€ RN таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.TABLENAME = :old.TABLENAME ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ TABLENAME таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.VERSION = :old.VERSION ) and :new.COMPANY is null ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ VERSION таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.COMPANY = :old.COMPANY ) and :new.VERSION is not null ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ COMPANY таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.TABLERN = :old.TABLERN ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ TABLERN таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.AUTHID = :old.AUTHID ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ AUTHID таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.MODIFDATE = :old.MODIFDATE ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ MODIFDATE таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.OPERATION = :old.OPERATION ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ OPERATION таблицы UPDATELIST недопустимо.' );
end if;
if ( not( :new.NOTE = :old.NOTE ) ) then
P_EXCEPTION( 0,'»зменение значени€ пол€ NOTE таблицы UPDATELIST недопустимо.' );
end if;
end;
/
SHOW ERRORS;
