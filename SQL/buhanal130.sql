...
UNION ALL  
select 'UPDATE sveta.sf_poz_vyr SET buhanal=130 WHERE kod_prod='||TO_CHAR(list_130.kod_prod)||
       ' AND id_poz_prod='||TO_CHAR(sf_poz_vyr.id_poz_prod)||' AND kol='||TO_CHAR(list_130.kol)||' and kod_tu=6;'
from sveta.sf_poz_vyr,list_130 
where sf_poz_vyr.kod_prod=list_130.kod_prod
  and sf_poz_vyr.kol=list_130.kol
  and sveta.sf_poz_vyr.buhanal=1
UNION ALL
SELECT
'-- ќбновить BUHANAL в таблице SF_POZ_PROD - в позици€х с совпадающим весом' FROM dual
UNION ALL  
select 'UPDATE sveta.sf_poz_prod SET buhanal=130 WHERE kod_prod='||TO_CHAR(list_130.kod_prod)||
       ' AND id_poz_prod='||TO_CHAR(sf_poz_prod.id_poz_prod)||' AND kol='||TO_CHAR(list_130.kol)||' and kod_tu=6;'
from sveta.sf_poz_prod,list_130 
where sf_poz_prod.kod_prod=list_130.kod_prod
  and sf_poz_prod.kol=list_130.kol
...
UNION ALL
select DISTINCT 'DELETE FROM sveta.sf_poz_prod WHERE kod_prod='||TO_CHAR(list_130.kod_prod)||
       ' AND id_poz_prod='||TO_CHAR(sf_poz_prod.id_poz_prod)||' and kod_tu=6;'
from sveta.sf_poz_prod,list_130,sveta.sf_poz_vyr 
where sf_poz_prod.kod_prod=list_130.kod_prod
  and sf_poz_prod.kol<>list_130.kol
...
select DISTINCT 
 'INSERT INTO sveta.sf_poz_prod (ID_POZ_PROD, EDIZM, STUSL, KOD_PROD, KOD_TU, NAIM_TOV, KOL, CENA, AKCIZ1, SUMMA, AKCIZ2, STNDS, '||
  ...
  'VALUES('||
    TO_CHAR(sf_poz_prod.ID_POZ_PROD*10+DECODE(sf_poz_vyr.kol,list_130.kol,1,0))||','||
	...
	TO_CHAR(DECODE(sf_poz_vyr.kol,list_130.kol,130,sf_poz_vyr.BUHANAL))||','|| 
