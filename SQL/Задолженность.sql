CREATE OR REPLACE VIEW MASTER.V_UNP_BUDJET_NPR
AS 
...
SELECT /*+ ORDERED USE_NL(bills,pay,r) */
  ...
  (CASE 
     WHEN bills.date_kvit<r.begin_date AND bills.date_plat>=r.begin_date AND bills.date_plat<=r.end_date THEN bills.LUK_SUMMA_DOK-NVL(pay.begin_summa,0)
	 ELSE 0
   END) AS begin_deb_cur, -- Задолженность прошлого периода к погашению в текущем
  (CASE 
     WHEN bills.date_kvit<r.begin_date AND bills.date_plat>r.end_date THEN bills.LUK_SUMMA_DOK-NVL(pay.begin_summa,0)
	 ELSE 0
  ...
  (CASE 
     WHEN bills.date_kvit>=r.begin_date AND bills.date_plat<=r.end_date THEN bills.LUK_SUMMA_DOK
	 ELSE 0
   END) AS summa_dok_cur, -- Отгружено в текущем периоде, к погашению в текущем периоде
  (CASE 
     WHEN date_kvit>=r.begin_date AND bills.date_plat>r.end_date THEN bills.LUK_SUMMA_DOK 
	 ELSE 0
  ...
  (CASE 
     WHEN bills.date_plat<=r.end_date THEN bills.LUK_SUMMA_DOK-NVL(pay.end_summa,0)
	 ELSE 0
